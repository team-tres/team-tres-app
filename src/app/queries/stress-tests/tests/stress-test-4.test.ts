...
describe('Stress Test 4 - Edge Cases', () => {
  it('handles 0% increase in expenses', () => {
    const data = { increaseInExpenses: [1000, 2000], expenseIncreasedPercentage: 0 };
  ...
  it('handles 100% increase in expenses', () => {
    const data = { increaseInExpenses: [1000, 2000], expenseIncreasedPercentage: 1 };
    const result = calculateStressTest4(data);
    expect(result.stressEffects.every((v, i) => v === data.increaseInExpenses[i])).toBe(true);
  });

  it('handles empty expenses array', () => {
    const data = { increaseInExpenses: [], expenseIncreasedPercentage: 0.5 };
    const result = calculateStressTest4(data);
